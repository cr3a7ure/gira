...
[alias]
  ...
  giboard = "!f() {\
    local todo=$(printf %-5s $(grep \"status: TODO\" STATUS.md -c ));\
    local wip=$(printf %-5s $(grep \"status: WIP\" STATUS.md -c ));\
    local blok=$(printf %-5s $(grep \"status: BLOK\" STATUS.md -c ));\
    local blog=$(printf %-5s $(grep \"status: BLOG\" STATUS.md -c ));\
    local done=$(printf %-5s $(grep \"status: DONE\" STATUS.md -c ));\
    echo \"\
 | TODO | WIP  | BLOK | BLOG | DONE |\n\
 |------|------|------|------|------|\n\
 | $todo| $wip| $blok| $blog| $done|\
    \";\
    sed -i \"/| TODO |*/,+3d\" STATUS.md;\
    line_n=$(grep '## BOARD' STATUS.md -n | cut -d: -f1);\
    sed -i \" $((line_n+1))a \\\
| TODO | WIP  | BLOK | BLOG | DONE |\\n\
|------|------|------|------|------|\\n\
| $todo| $wip| $blok| $blog| $done|\
